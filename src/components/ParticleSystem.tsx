...
const getPointOnCurve = (t: number, type: string) => {
  ...
  switch (type) {
    ...
    case 'rose':
      const k = 4;
      x = Math.cos(k * t) * Math.cos(t) * 10;
      y = Math.cos(k * t) * Math.sin(t) * 10;
      break;
    case 'lemniscate':
      const a2 = 10;
    ...
    case 'koch':
      ...
      const r = 10 * (1 + 0.3 * Math.sin(6 * t));
      x = r * Math.cos(t);
      y = r * Math.sin(t);
      break;
    case 'catenary':
      x = t * 2;
      y = Math.cosh(t / 2) * 2 - 10;
      break;
    case 'vortex':
    default:
      const radius = t * 0.5;
...
export const ParticleSystem = () => {
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      ...
      positions[i * 3] += (targetX - positions[i * 3]) * 0.02 + noise * 0.01;
      positions[i * 3 + 1] += (targetY - positions[i * 3 + 1]) * 0.02 + noise * 0.01;
      positions[i * 3 + 2] += (targetZ - positions[i * 3 + 2]) * 0.02 + noise * 0.01;
    }
